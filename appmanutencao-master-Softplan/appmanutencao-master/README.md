...
Solução: Após deletar um par, não deve processar um Next pois esse registro "pulado" não será processado. Se eventualmente houver dois pares consecutivos só o primeiro será apagado. Então, deve-se executar Delete ou Next, nunca os dois. Adicionado clausula Else para corrigir o problema.
...
Solução: Sinceramente não compreendi muito bem essa solicitação. Se o objetivo final é ter um TClientDataSet em memória com tantos registros temos um problema no qual eu não vislumbro solução (se tem gostaria de conhecer, sério, rs). Se o objetivo é tão somente o envio dos arquivos, implementei uma sub procedure que ira manipular apenas um registro por vez. Ao término de cada iteração elimino da memória o registro (TDataSet).
...
Solução: Tentativa de manipular um DataSet fechado por ter passado "Null" como parâmetro (simulação). Implementado Try Except com loop regressivo apagando os arquivo eventualmente gerados. Poderia melhorar adicionando num TStringList apenas os arquivos criados e só apagar os arquivos do TStringList. Mas por ser teste implementei um loop simples.